...
def load_model(model_id, device="cuda", quantization_bits=8):
    ...
    Args:
        quantization_bits: 2, 4, 8, or None (for FP16)
    """
    ...
    if "gpt-oss-120b" in str(model_id).lower():
        ...
        quantization_config = BitsAndBytesConfig(
            ...
            bnb_4bit_quant_storage=torch.uint8  # 2-bit storage
        )
        model = AutoModelForCausalLM.from_pretrained(
            model_id,
            device_map="auto",
...
def main():
    ...
    for mname, (model, tokenizer) in models.items():
        ...
        response_col = f"{model_short_name}_Response"
        score_col = f"{model_short_name}_Score"
